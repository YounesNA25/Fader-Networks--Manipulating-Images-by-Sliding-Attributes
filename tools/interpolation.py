...
def parse_args():
    ...
    parser.add_argument(
        "--encoder-fpath",
        default="trained models/smiling_encoder_60.pt",
        type=str,
        help="file path to load the encoder",
    )
    parser.add_argument(
        "--decoder-fpath",
        default="trained models/smiling_decoder_60.pt",
        type=str,
        help="file path to load the decoder",
    )
    parser.add_argument(
        "--discriminator-fpath",
        default="trained models/smiling_discriminator_60.pt",
        type=str,
...
def grid_one_epoch(arg, encoder, decoder, discriminator, test_data_loader):
    ...
    for batch_idx, (batch_x, batch_y) in enumerate(
        ...
        if batch_idx % arg.sample_batch == 0:
            ...
            # Save the figure
            save_dir = f".\Results\grid\{arg.attr_chg[y2flip_idx[0]]}"
